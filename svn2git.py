...
class Svn2GitException(Exception):
    pass
...
def get_last_revision_from_svn(svn_url):
    ...
    return Svn2GitException("svn info {} output did not specify the current revision".format(svn_url))
...
def get_svn_info_from_git(git_dir):
    ...
    return Svn2GitException("git log -1 HEAD --pretty=%b output did not specify the current revision")
...
def sync_git_mirror(git_repo, cache_dir, new_svn_url=None):
    if cache_dir:
        os.makedirs(cache_dir, exist_ok=True)
        cache_path = os.path.join(cache_dir, "cache." + git_repo.replace("/", ".") + ".tar")
    ...
    with tempfile.TemporaryDirectory(prefix="svn2git-") as tmp_dir:
        ...
        if new_svn_url:
            if not is_repo_empty(git_dir):
                raise Svn2GitException("Specifed new_svn_url, but the destination repo is not empty")
...
def main():
    parser = argparse.ArgumentParser(description="SVN repositories to Git")
    ...
    subparser_import = subparsers.add_parser("import", help="Import SVN repository to the Git repo")
    subparser_import.add_argument("git_repo", metavar="GIT_REPO", help="Git repo in format: git@xxx:groupname/projectname.git")
    subparser_import.add_argument("svn_url", metavar="SVN_URL", help="SVN repository to import")

    subparser_update = subparsers.add_parser("update", help="Update the Git repository from SVN")
    subparser_update.add_argument("git_repo", metavar="GIT_REPO", help="Git repo in format: git@xxx:groupname/projectname.git")
    args = parser.parse_args(sys.argv[1:] or ["--help"])

    new_svn_url = args.svn_url if "svn_url" in args else None
    sync_git_mirror(args.git_repo, args.cache_dir, new_svn_url=new_svn_url)
